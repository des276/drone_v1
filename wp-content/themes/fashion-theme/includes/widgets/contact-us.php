...
class Contact_Us_Widget extends WP_Widget {
	...
	function Contact_Us_Widget() {
		...
		$this->WP_Widget( 'Contact_Us_Widget', __('Leetheme Contact Us', 'ltheme_domain'), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		...
		if ( empty( $instance['contact_address'] ) || ! $contact_address = $instance['contact_address'] )
 		...
 		if ( empty( $instance['contact_email'] ) || ! $contact_email = $instance['contact_email'] )
 			$contact_email = 'Email...';
 		if ( empty( $instance['contact_share'] ) || ! $contact_share = $instance['contact_share'] )
 			$contact_share = '';
 		if ( empty( $instance['contact_icon_style'] ) || ! $contact_icon_style = $instance['contact_icon_style'] )
			...
			<ul class="contact-information">
				...
				<?php } ?>
				<li class="media">
					<div class="contact-icon <?php if (isset($contact_icon_style)) echo esc_attr($contact_icon_style); ?>"><i class="fa fa-paper-plane"></i></div>
					<div class="contact-text"><span><?php if (isset($contact_address)) echo esc_attr($contact_address); ?></span></div>
				</li>
				<li class="media">
					<div class="contact-icon <?php if (isset($contact_icon_style)) echo esc_attr($contact_icon_style); ?>"><i class="fa fa-phone"></i></div>
					<div class="contact-text"><span><?php if (isset($contact_phone)) echo esc_attr($contact_phone); ?></span></div>
				</li>
				<li class="media">
					<div class="contact-icon <?php if (isset($contact_icon_style)) echo esc_attr($contact_icon_style); ?>"><i class="fa fa-envelope"></i></div>
					<div class="contact-text"><span><?php if (isset($contact_email)) echo esc_attr($contact_email); ?></span></div>
				</li>
				<li class="media">
					<?php if (isset($contact_share)) echo do_shortcode(''.$contact_share.'');  ?>
				</li>
			</ul>
<?php 
			echo $after_widget;
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['contact_share'] = $new_instance['contact_share'];
	...
	function form( $instance ) {
		...
		$contact_share     = isset( $instance['contact_share'] ) ? esc_attr( $instance['contact_share'] ) : '';
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'contact_share' )); ?>"><?php _e( 'Share:', 'ltheme_domain' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'contact_share' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'contact_share' )); ?>" type="text" value="<?php echo esc_attr($contact_share); ?>" /></p>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'contact_icon_style' )); ?>"><?php _e( 'Contact icon style:', 'ltheme_domain' ); ?></label>
			<select name="<?php echo esc_attr($this->get_field_name( 'contact_icon_style' )); ?>" id="<?php echo esc_attr($this->get_field_name( 'contact_icon_style' )); ?>" type="text">
				<option value='' <?php echo ($contact_icon_style == '') ? 'selected' : ''; ?>>Normal</option>
				<option value='border-circle' <?php echo ($contact_icon_style == 'border-circle') ? 'selected' : ''; ?>>Border Circle</option>
			</select>
		</p>
<?php
	}
